...
function createResolve(opts) {
  ...
  const {preferTsExts, tsNodeExperimentalSpecifierResolution, extensions, resolveIdByPaths} = opts;
  ...
  */
  ...
  function packageResolve(specifier, base, conditions) {
    let { packageName, packageSubpath, isScoped } =
  ...
  function defaultResolve(specifier, context = {}, defaultResolveUnused) {
    try {
      ...
      if (specifier.indexOf('.') < 0) {
        const mapped = resolveIdByPaths(specifier);
        if (mapped) {
          specifier = mapped;
        }
      }
